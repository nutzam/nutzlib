...
public class Castors {
	...
	public <F, T> T cast(Object src, Class<F> fromType, final Class<T> toType, String... args)
			throws FailToCastObjectException {
		if (null == src) {
		...
		if (from.canCastToDirectly(toType)) // Use language built-in cases
			return (T) src;
		try {
			DefaultCastor castor = new DefaultCastor(new SimpleDateFormat("yyyy-MM-dd"),
					new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"),new SimpleDateFormat("HH:mm:ss"));
			Class<?> myClass = toType;
			if(toType.isPrimitive())
				myClass = Mirror.me(toType).getWrapperClass();
			if(String.class.equals(fromType))
				return (T) castor.cast((String)src, myClass, args);
			if(String.class.equals(toType))
				return (T) castor.cast(src, String.class, args);
			return (T) castor.cast(src, myClass, args);
		} catch (FailToCastObjectException e) {
			throw e;
		} catch (Exception e) {
			Logs.getLog(getClass()).debug("Cast Exception", e);
